...
describe('stream cipher', () => {
  ...
  it('zuc-eea3', () => {
    ...
    const k = new Uint8Array([0x17, 0x3D, 0x14, 0xBA, 0x50, 0x03, 0x73, 0x1D, 0x7A, 0x60, 0x04, 0x94, 0x70, 0xF0, 0x0A, 0x29])
    const m = new Uint8Array([0x6C, 0xF6, 0x53, 0x40, 0x73, 0x55, 0x52, 0xAB, 0x0C, 0x97, 0x52, 0xFA, 0x6F, 0x90, 0x25, 0xFE, 0x0B, 0xD6, 0x75, 0xD9, 0x00, 0x58, 0x75, 0xB2])
    const c = new Uint8Array([0xA6, 0xC8, 0x5F, 0xC6, 0x6A, 0xFB, 0x85, 0x33, 0xAA, 0xFC, 0x25, 0x18, 0xDF, 0xE7, 0x84, 0x94, 0x0E, 0xE1, 0xE4, 0xB0, 0x30, 0x23, 0x8C, 0xC8])
  ...
  it('zuc-eia3', () => {
    ...
    const r1 = new Uint8Array([0xC8, 0xA9, 0x59, 0x5E])
    const t2: ZUCParams = {
      KEY: new Uint8Array([0x47, 0x05, 0x41, 0x25, 0x56, 0x1E, 0xB2, 0xDD, 0xA9, 0x40, 0x59, 0xDA, 0x05, 0x09, 0x78, 0x50]),
      M: new Uint8Array(12),
      COUNTER: 0x561EB2DD,
      BEARER: 0x14,
      DIRECTION: 0,
...
describe('block cipher', () => {
  ...
  it('camellia-128', () => {
    const k = new Uint8Array([0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10])
    const m = new Uint8Array([0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10])
    const c = new Uint8Array([0x67, 0x67, 0x31, 0x38, 0x54, 0x96, 0x69, 0x73, 0x08, 0x57, 0x06, 0x56, 0x48, 0xEA, 0xBE, 0x43])
    const cipher = camellia(k)
    expect(cipher.encrypt(m)).toMatchObject(c)
    expect(cipher.decrypt(c)).toMatchObject(m)
  })
  it('camellia-192', () => {
    const k = new Uint8Array([0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77])
    const m = new Uint8Array([0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10])
    const c = new Uint8Array([0xB4, 0x99, 0x34, 0x01, 0xB3, 0xE9, 0x96, 0xF8, 0x4E, 0xE5, 0xCE, 0xE7, 0xD7, 0x9B, 0x09, 0xB9])
    const cipher = camellia(k)
    expect(cipher.encrypt(m)).toMatchObject(c)
    expect(cipher.decrypt(c)).toMatchObject(m)
  })
  it('camellia-256', () => {
    const k = new Uint8Array([0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF])
    const m = new Uint8Array([0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10])
    const c = new Uint8Array([0x9A, 0xCC, 0x23, 0x7D, 0xFF, 0x16, 0xD7, 0x6C, 0x20, 0xEF, 0x7C, 0x91, 0x9E, 0x3A, 0x75, 0x09])
    const cipher = camellia(k)
    expect(cipher.encrypt(m)).toMatchObject(c)
    expect(cipher.decrypt(c)).toMatchObject(m)
  })
