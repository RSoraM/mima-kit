...
import { aria } from '../src/cipher/aria'
...
import { arc5 } from '../src/cipher/arc5'
...
describe('block cipher', () => {
  ...
  // * ARIA
  it('aria-128', () => {
    const k = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F])
    const m = new Uint8Array([0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF])
    const c = new Uint8Array([0xD7, 0x18, 0xFB, 0xD6, 0xAB, 0x64, 0x4C, 0x73, 0x9D, 0xA9, 0x5F, 0x3B, 0xE6, 0x45, 0x17, 0x78])
  ...
  it('aria-192', () => {
    const k = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17])
    const m = new Uint8Array([0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF])
    const c = new Uint8Array([0x26, 0x44, 0x9C, 0x18, 0x05, 0xDB, 0xE7, 0xAA, 0x25, 0xA4, 0x68, 0xCE, 0x26, 0x3A, 0x9E, 0x79])
  ...
  it('aria-256', () => {
    const k = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F])
    const m = new Uint8Array([0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF])
    const c = new Uint8Array([0xF9, 0x2B, 0xD7, 0xC7, 0x9F, 0xB7, 0x2E, 0x2F, 0x2B, 0x8F, 0x80, 0xC1, 0x97, 0x2D, 0x24, 0xFC])
  ...
  it('rc5-8/12/4', () => {
    ...
    const k = new Uint8Array([0x00, 0x01, 0x02, 0x03])
    const m = new Uint8Array([0x00, 0x01])
    const c = new Uint8Array([0x21, 0x2A])
  ...
  it('rc5-16/16/8', () => {
    // source: https://datatracker.ietf.org/doc/html/draft-krovetz-rc6-rc5-vectors-00
    const k = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07])
    const m = new Uint8Array([0x00, 0x01, 0x02, 0x03])
    const c = new Uint8Array([0x23, 0xA8, 0xD7, 0x2E])
  ...
  it('rc5-32/20/16', () => {
    // source: https://datatracker.ietf.org/doc/html/draft-krovetz-rc6-rc5-vectors-00
    const k = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F])
    const m = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07])
    const c = new Uint8Array([0x2A, 0x0E, 0xDC, 0x0E, 0x94, 0x31, 0xFF, 0x73])
  ...
  it('rc5-64/24/24', () => {
    // source: https://datatracker.ietf.org/doc/html/draft-krovetz-rc6-rc5-vectors-00
    const k = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17])
    const m = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F])
    const c = new Uint8Array([0xA4, 0x67, 0x72, 0x82, 0x0E, 0xDB, 0xCE, 0x02, 0x35, 0xAB, 0xEA, 0x32, 0xAE, 0x71, 0x78, 0xDA])
  ...
  it('rc5-128/28/32', () => {
    // source: https://datatracker.ietf.org/doc/html/draft-krovetz-rc6-rc5-vectors-00
    const k = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F])
    const m = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F])
    const c = new Uint8Array([0xEC, 0xA5, 0x91, 0x09, 0x21, 0xA4, 0xF4, 0xCF, 0xDD, 0x7A, 0xD7, 0xAD, 0x20, 0xA1, 0xFC, 0xBA, 0x06, 0x8E, 0xC7, 0xA7, 0xCD, 0x75, 0x2D, 0x68, 0xFE, 0x91, 0x4B, 0x7F, 0xE1, 0x80, 0xB4, 0x40])
