 ...
 */
function toBigInt(n: Uint8Array): bigint {
  let v = 0n
  n.forEach(x => v = (v << 8n) + BigInt(x))
  return v
}
 ...
 */
function isProbablePrime(n: bigint, t: number): boolean {
  if (t <= 0)
    return false
  // 偶数
  if ((n & 1n) === 0n)
...
export function genPrime(b: number): bigint {
  ...
  const value = new Uint8Array(b >> 3)
  crypto.getRandomValues(value)
  let r = toBigInt(value)
  while (isProbablePrime(r, 7) === false) {
    r++
  }
  return r
}
