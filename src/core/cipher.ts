...
export function createCipher(algorithm: Cipher | IVCipher, description: BlockCipherInfo | StreamCipherInfo | IVStreamCipherInfo) {
...
export function createPadding(doPad: DoPad, unPad: UnPad, description: PaddingInfo): Padding {
  return wrap((M: Uint8Array, BLOCK_SIZE?: number) => (typeof BLOCK_SIZE === 'number' ? doPad(M, BLOCK_SIZE) : unPad(M)), description)
}
